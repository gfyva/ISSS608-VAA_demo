...
![](images/Basic%20concepts%20of%20animation.png){width="568"}

### **4.1.2 Terminology**

Before we dive into the steps for creating an animated statistical graph, it is important to understand some of the key concepts and terminology related to this type of visualization.
...
To overcome this problem, `theme(legend.position='none')` should be used as shown in the plot and code chunk below.
...
#### **1) Exploring other animations of bubble plot**
...
#### **2) Creating another variation of bubble plot using plot_ly()**

Tried out these few adjustments to the bubble plot:

- Tooltip displayed more attributes (e.g. country, year, population)

- Fixed axis ranges for animation

- Marker opacity reduced to introduce transparency in overlapping bubbles
...
bp <- globalPop %>%
  ...
  layout(
    xaxis = list(title = "Old Dependency Ratio", range = c(0, 100)),
    yaxis = list(title = "Young Dependency Ratio", range = c(0, 100))
  ) %>%
