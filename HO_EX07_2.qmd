...
```{r}
pacman::p_load(tidytext, widyr, wordcloud, DT, ggwordcloud, textplot, lubridate, hms, tidyverse, tidygraph, ggraph, igraph, dplyr, scales) 
...
raw_text <- tibble(folder = 
  ...
  transmute(newsgroup = basename(folder), 
            id, text)
write_rds(raw_text, "chap29/data/rds/news20.rds") 
...
```{r}
raw_text <- read_rds("chap29/data/rds/news20.rds")
raw_text %>%
  group_by(newsgroup) %>%
...
![](images/Screenshot%202026-06-04%20165619.png){width="567"}

### **15.6.1 Removing header and automated email signatures**
...
![](images/Screenshot%202026-06-04%20183156.png){width="442"}
...
### **15.7.3 Visualising tf-idf within newsgroups**
...
### **15.7.4 Counting and correlating pairs of words with the widyr package**
...
![](images/Screenshot%202026-06-04%20214418.png){width="520"}
...
### **15.7.5 Visualising correlation as a network**
...
### **15.7.9 Counting the bigram again**
...
### **15.7.10 Create a network graph from bigram data frame**
...
### **15.7.11 Visualizing a network of bigrams with ggraph**
...
**Newsgroup Similarity Analysis**

This analysis compares newsgroups based on their TF-IDF word profiles to identify communities with similar vocabulary usage.
...
```{r}
# Step 2: Compute pairwise similarity between newsgroups
newsgroup_similarity <- tf_idf %>%
  pairwise_cor(
    item = newsgroup,
    feature = word,
    value = tf_idf,
...
```{r}
# Step 3: View similarity table
newsgroup_similarity %>%
...
```{r}
# Step 4: Create heatmap of strongest similarities
top_similarity <- newsgroup_similarity %>%
  arrange(desc(correlation)) %>%
  slice_head(n = 30)
...
ggplot(
  ...
  labs(
    title = "Similarity Between Newsgroups",
    subtitle = "Top 30 most similar newsgroup pairs based on TF-IDF word profiles",
    x = "Newsgroup",
    y = "Newsgroup",
    fill = "Similarity"
  ) +
...
```{r}
# Step 5: Create network graph of similar newsgroups
similarity_graph <- top_similarity %>%
  graph_from_data_frame(directed = FALSE)

set.seed(1234)

ggraph(similarity_graph, layout = "fr") +
  geom_edge_link(
  ...
  labs(
    title = "Newsgroup Similarity Network",
    subtitle = "Edges show stronger similarity in TF-IDF word usage"
  ) +
  theme_void()
...
- **Technology-related newsgroups form the largest and most densely connected cluster**, indicating substantial overlap in technical vocabulary and discussion topics.

-  **Political discussion groups are closely related**, suggesting shared themes despite focusing on different political issues.

-  **Religion-related newsgroups exhibit strong similarity**, reflecting common discussions surrounding faith, beliefs, and religious concepts.

- **Sports newsgroups show moderate similarity**, with hockey and baseball communities sharing a common sports-related vocabulary.

-  **Some groups remain relatively isolated**, indicating more specialized or distinctive discussion topics.

**Implication**

-  TF-IDF similarity analysis reveals hidden relationships between communities that are not immediately apparent from the original newsgroup labels.

-  Related communities naturally cluster together, demonstrating the effectiveness of text mining in uncovering broader thematic structures.
